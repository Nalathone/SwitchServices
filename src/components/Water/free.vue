<template>
    ...
    <!-- End Top content -->
          ...
          <div class="mt-3">
      <span id="font-blue" class="font-bold text-2xl">Water Switch Services</span>
  </div>
        ...
        <div v-for="(province, index) in provinces" :key="province[0]">
          <div class="mb-5 bg-gray-100 rounded-lg shadow-md text-center p-10 m-0 w-full">
            ...
            <p>
              ...
              {{ province[1] }} <br> {{ province[2] }} - {{ province[4] }}
            </p>
  ...
  export default {
    setup() {
      ...
      const sendRequest = (province, value) => {
        // const url = `http://172.21.25.79:7007/jdb/v1/disableenablewaterarea?sign=ed0e999dda8e923cb1eaa402384213a3439db0fe556e68ba24a8a985947f14dd&PRO_ID=01&SHORT_NAME=VC&status=C`;
        const url = `http://172.21.25.79:7007/jdb/v1/disableenablewaterarea?sign=ed0e999dda8e923cb1eaa402384213a3439db0fe556e68ba24a8a985947f14dd&PRO_ID=${province[0]}&SHORT_NAME=${province[3]}&status=${value}`;
        axios.post(url)
          .then(response => {
      ...
      onMounted(async () => {
        ...
        const response = await axios.post("http://172.21.25.79:7007/jdb/v1/displaywateraccount?sign=ed0e999dda8e923cb1eaa402384213a3439db0fe556e68ba24a8a985947f14dd");
        provinces.value = response.data.data.reduce((result, value, index, array) => {
          if (index % 5 === 0) {
